...
async function initAuthorPage() {
    try {
        // 从URL获取作者名称
        const authorName = window.UrlHandler.getAuthorNameFromUrl();
        if (!authorName) {
            showErrorMessage('Author not found. Please check the URL.');
            return;
        }

        pageState.authorName = authorName;
        ...
        // 获取作者ID
        // 首先尝试从 URL 中获取 ID
        const authorId = window.UrlHandler.getAuthorIdFromUrl();

        if (authorId) {
            // 直接使用 URL 中的 ID
            console.log('Using author ID from URL:', authorId);
            pageState.authorId = authorId;
        } else {
            // 如果 URL 中没有 ID，则通过名称查询
            console.log('Getting author by name:', authorName);
            try {
                ...
                console.error('Error getting author by name:', authorError);
                showErrorMessage(`Failed to get author "${authorName}". Please try refreshing.`);
                return;
            }
        }

        // 更新页面标题和作者信息
        updateAuthorInfo(authorName);
 ...
 */
function updatePageMetadata(authorName) {
    document.title = `${authorName} Quotes - Quotes Collection`;

    // 更新meta描述
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
        metaDescription.content = `Explore our collection of quotes by ${authorName}. Find inspiration, wisdom, and insights from ${authorName}'s most famous sayings.`;
    }

    // 更新canonical链接
    const canonicalLink = document.querySelector('link[rel="canonical"]');
    if (canonicalLink) {
        canonicalLink.href = `https://quotese.com/authors/${UrlHandler.slugify(authorName)}-${pageState.authorId}.html`;
    }

    // 添加Person结构化数据
    addPersonStructuredData(authorName, pageState.authorId);

    // 更新社交媒体元数据
    if (window.SocialMetaUtil) {
        window.SocialMetaUtil.updateAuthorMetaTags(
            { name: authorName, id: pageState.authorId },
...
function updateAuthorInfo(authorName) {
    ...
    if (initialElement) {
        initialElement.textContent = authorName.charAt(0);
    }

    // 更新作者简介（如果有）
    const bioElement = document.getElementById('author-bio');
    if (bioElement) {
        bioElement.textContent = `Explore the wisdom and insights of ${authorName} through our collection of quotes.`;
    }
}
...
async function loadPageComponents() {
    ...
    await ComponentLoader.loadComponent('quotes-container', 'quotes-list', {
        title: `Quotes by ${pageState.authorName}`
    });
...
function renderQuotes(quotes) {
    ...
    window.QuoteCardComponent.renderList(quotes, 'quotes-list', {
        ...
        emptyMessage: `No quotes found by ${pageState.authorName}.`
    });
}
...
function renderAuthors(authors) {
    ...
    randomAuthors.forEach(author => {
        ...
        if (author.name === pageState.authorName) {
        ...
        authorItem.className = className;
        authorItem.innerHTML = `
            <div class="flex items-start space-x-2 w-full">
                <div class="flex-grow pr-2">
                    <a href="${UrlHandler.getAuthorUrl(author)}" class="${author.name === pageState.authorName ? 'text-yellow-600 dark:text-yellow-400 font-semibold' : 'hover:text-yellow-600 dark:hover:text-yellow-400'} font-medium transition-colors duration-300" title="${author.name}">${author.name}</a>
                </div>
...
function updateQuoteCount(count) {
    ...
    const quotesListTitle = document.getElementById('quotes-list-title');
    if (quotesListTitle) {
        quotesListTitle.textContent = `Quotes by ${pageState.authorName}`;
        console.log('Updated quotes-list-title to:', `Quotes by ${pageState.authorName}`);
    } else {
    ...
    if (quoteCountElement) {
        ...
        if (parentElement) {
            parentElement.innerHTML = `<span id="quote-count">${count}</span> quotes by this author`;
 ...
 */
function addPersonStructuredData(authorName, authorId) {
    // 移除旧的结构化数据
    const oldScript = document.getElementById('person-structured-data');
    if (oldScript) {
        oldScript.remove();
    }
    ...
    const structuredData = {
        ...
        "mainEntityOfPage": {
            "@type": "WebPage",
            "@id": `https://quotese.com/authors/${UrlHandler.slugify(authorName)}-${authorId}.html`
        }
    };

    // 如果有引用数量，添加到结构化数据中
    if (pageState.totalQuotes > 0) {
        structuredData.description = `${authorName} has ${pageState.totalQuotes} famous quotes in our collection.`;
    }
